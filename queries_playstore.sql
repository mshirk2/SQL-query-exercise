...
-- 5. Find the app that has the most reviews with a rating greater than equal to 4.8.
    SELECT app_name, reviews, rating FROM analytics WHERE rating >= 4.8 ORDER BY reviews DESC L
IMIT 1;
...
-- 8. Find all apps with a min install not exceeding 50, that have a rating. Order your results by highest rated first.
    SELECT app_name, rating, min_installs FROM analytics WHERE min_installs < 50 ORDER BY ratin
g DESC;
...
-- 10. Find the top 10 most-reviewed apps that cost between 10 cents and a dollar.
    SELECT app_name, reviews, price FROM analytics WHERE price BETWEEN .1 AND 1 ORDER BY review
s DESC LIMIT 10;
...
-- 14. Find all the categories that have more than 300 apps in them.
    SELECT COUNT(*), category FROM analytics GROUP BY category HAVING COUNT(*) > 300;
